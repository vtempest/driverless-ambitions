...
export async function getMetrics(c: RouteContext): Promise<Response> {
  const who = await authenticate(c.request, c.env, "reader");
  const [report, fleet, scores, coverage] = await Promise.all([
    ...
    coverageFor(c.env, who.tenant, 3),
  ]);
  const extra: Record<string, number> = {
    atlas_fleet_devices: fleet?.devices || 0,
    atlas_fleet_devices_online: fleet?.online || 0,
    atlas_runs_scored: scores?.scored || 0,
    atlas_coverage_cells: coverage.coverage.totals.cells,
    atlas_coverage_cells_covered: coverage.coverage.totals.covered_cells,
    atlas_coverage_cells_gap: coverage.coverage.totals.gap_cells,
    atlas_coverage_pct: coverage.coverage.totals.coverage_pct,
    atlas_dataset_scenes: coverage.catalog.accepted_runs,
  };
