...
export function openApiDocument(baseUrl: string, appName: string): Record<string, unknown> {
  return {
    ...
    paths: {
      ...
      "/api/coverage": { get: op("ODD coverage matrix (visibility x lighting), route-class totals and the cells that are still gaps", "reader", { parameters: [q("min_runs", { type: "integer" })] }) },
      "/api/plan": {
        get: op("Dataset-scale and GPU-budget plan: scenes to render, storage and low/mid/high cost bands at marketplace GPU rates", "reader", {
          parameters: [
            q("tier", { type: "string", enum: ["poc", "adaptation", "robust"] }),
            q("scenes", { type: "integer" }),
            q("have", { type: "integer" }),
            q("workload", { type: "string", enum: ["finetune", "medium", "scratch"] }),
            q("gpu", { type: "string", enum: ["a100_80gb", "h100_80gb", "l40s", "rtx4090"] }),
            q("interruptible", { type: "boolean" }),
            q("seconds_per_scene", { type: "number" }),
            q("resolution", { type: "string", enum: ["720p", "1080p", "1440p", "4k"] }),
            q("camera_streams", { type: "integer" }),
            q("mb_per_scene", { type: "number" }),
            q("rate", { type: "number" }),
            q("storage_rate", { type: "number" }),
            q("preprocess_overhead", { type: "number" }),
            q("min_runs", { type: "integer" }),
          ],
        }),
      },
