...
export function openApiDocument(baseUrl: string, appName: string): Record<string, unknown> {
  return {
    ...
    paths: {
      ...
      "/api/gpu-prices": { get: op("Latest marketplace GPU quotes (Vast.ai, RunPod) per GPU class and pricing mode, with the price history behind the planner's chart — market data, no tenant in it", "none") },
      "/api/gpu-prices/refresh": { post: op("Poll the marketplaces now instead of waiting for the nightly cron", "admin") },
      "/api/coverage": {
