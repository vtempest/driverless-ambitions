...
export function openApiDocument(baseUrl: string, appName: string): Record<string, unknown> {
  return {
    ...
    paths: {
      ...
      "/api/training/plan": { get: op("Training-set inventory, sizing tiers, condition coverage gaps and a low/typical/high GPU budget; format=markdown returns a budget memo", "reader", { parameters: [q("scenes", { type: "integer" }), q("workload", { type: "string", enum: ["finetune", "medium", "scratch"] }), q("gpu", { type: "string", enum: ["a100_80gb", "h100_pcie", "l40s_48gb", "rtx4090_24gb"] }), q("pricing", { type: "string", enum: ["on_demand", "interruptible"] }), q("rate_usd_hr", { type: "number" }), q("bytes_per_scene", { type: "integer" }), q("disk_usd_per_gb_month", { type: "number" }), q("retention_days", { type: "integer" }), q("preprocess_gpu_s_per_scene", { type: "number" }), q("format", { type: "string", enum: ["json", "markdown"] })] }) },
