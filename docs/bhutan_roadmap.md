...
| [x] | **Vast.ai / RunPod price APIs** ([vast-ai/vast-python](https://github.com/vast-ai/vast-python), [runpod/runpod-python](https://github.com/runpod/runpod-python)) | Nightly poll of both marketplaces' public listing endpoints, aggregated to min/p25/median $/GPU-hour per GPU class and pricing mode; the planner prices a run on the cheapest provider's median and says so, falling back to reference rates when the feed is stale | `dashboard/src/gpu_prices.ts`, `dashboard/src/routes/gpu_prices.ts`, Planner tab |
...
- [ ] [ziyc/drivestudio](https://github.com/ziyc/drivestudio) — Gaussian-splatting reconstruction of driving scenes with separated dynamic actors, so a reconstructed Bhutan drive can be re-rendered with different traffic, not just different weather. The cheaper half of the neurad-studio question.
- [ ] [PJLab-ADG/LimSim](https://github.com/PJLab-ADG/LimSim) — long-term interactive traffic simulation; the question is whether it generates the *behaviour* diversity (yielding, overtaking on a hairpin) the scenario library has to hand-author today.
...
- [x] [vast-ai/vast-python](https://github.com/vast-ai/vast-python) and [runpod/runpod-python](https://github.com/runpod/runpod-python) — the marketplaces' own clients. **Verdict: integrated without the SDKs.** Both publish the listing endpoint the SDKs wrap (Vast.ai `/api/v0/bundles/`, RunPod's GraphQL `gpuTypes`) as unauthenticated JSON, and a Worker cannot run a Python CLI; `src/gpu_prices.ts` calls the two endpoints directly. The SDKs stay relevant for the *launch* side, next to SkyPilot, not for prices.
...
- [ ] [pytorch/torchsnapshot](https://github.com/pytorch/torchsnapshot) and PyTorch Distributed Checkpoint — how cheap a checkpoint has to be for interruptible instances to pay off. Feeds the spot break-even item above; the planner currently assumes checkpointing is free.
- [ ] [opencost/opencost](https://github.com/opencost/opencost) — cost allocation per workload. Worth reading for its allocation model before building cost KPIs (dollars per accepted hour, per released clip) rather than for the code.
- [ ] [lancedb/lance](https://github.com/lancedb/lance) — columnar multimodal format with random access over object storage. The alternative to WebDataset shards when a training run wants to sample by ODD cell rather than stream sequentially, which is exactly what closing a coverage gap needs.
- [ ] [shadeform](https://docs.shadeform.ai/) and [TensorDock](https://github.com/tensordockllc) price APIs — two more marketplaces for the feed; the question is whether their listings are comparable enough to pool with Vast.ai and RunPod or need their own column.
...
- [x] Live GPU prices in the planner: Vast.ai and RunPod listings polled on the cron trigger, aggregated per GPU class and pricing mode, cached in D1 and priced into the plan with its provenance and a price-history chart (`/api/gpu-prices`).
- [ ] Run comparison: overlay two runs' timelines and event markers.
- [ ] Saved plans: name a plan, store it, and diff two plans (target size, GPU, cost) so a budget change is reviewable. Pin each saved plan to the price observation it used so the number can be reproduced after the market moves.
- [ ] Price alerts: watch a GPU class and notify (the webhook item below) when its median drops under a threshold, so a flexible training window can be booked on a dip.
- [ ] Spot break-even in the planner: given a checkpoint interval and a restart cost, say how much interruption a run can absorb before the 40 % interruptible discount stops paying — the number that decides on-demand versus bid.
- [ ] Marketplace storage and egress in the budget: Vast.ai publishes a per-listing `storage_cost`, already captured with each quote; use it and R2 egress instead of the typed $/GB-month default.
- [ ] Broaden the price feed beyond two marketplaces (Shadeform, TensorDock, Lambda) and show a "cheapest provider today" row per GPU class.
...
The planner's cost bands rested on four hard-coded hourly rates. That is the one
number in a pilot budget that is not ours to assume: the same H100 is listed near
$0.90/h on one Vast.ai host and above $2.00 on another, and an interruptible bid
is a third number again. This pass replaces the assumption with an observation.

* `dashboard/src/gpu_prices.ts` — parsers for the Vast.ai `/api/v0/bundles/`
  listing API and RunPod's GraphQL `gpuTypes` query, both public and key-free.
  A Vast.ai listing prices a whole machine, so `dph_total` and `min_bid` are
  divided by `num_gpus`; card names are matched to the planner's four GPU classes
  with memory disambiguating the 40 GB A100 from the 80 GB one; hosts below 0.9
  reliability and $0 listings are dropped. Offers collapse into one quote per
  provider, GPU class and pricing mode carrying min, 25th percentile and median.
  `fetchProviderOffers` never throws — a provider that is down contributes no
  offers rather than a wrong price.
* Migration `0003_gpu_prices.sql` — `gpu_price_quotes`, keyed by observation,
  provider, GPU and mode. Market data, so the rows carry no `tenant_id`.
* `dashboard/src/routes/gpu_prices.ts` — `GET /api/gpu-prices` (unauthenticated,
  like `/api/scenes`: there is no tenant data in a market price) serves the
  latest quote per GPU and mode plus the history behind the chart;
  `POST /api/gpu-prices/refresh` is admin-only and audited. The nightly cron
  records one observation per trigger, once for all tenants.
* Planner — `planProgram(input, rates)` prices a run on the **median of the
  cheapest provider**, not the cheapest listing, which is usually gone by the
  time a job starts. Three sources, always named in `compute.price_source`: an
  explicit `usd_per_gpu_hour` wins, then a live quote under 72 hours old, then
  the reference table. A marketplace bid price is already discounted, so the
  reference interruptible discount is not applied on top of it, and
  `live_prices=false` opts a plan out for reproducibility.
* Front end — a price-source line on the compute-cost tile ("market median of 14
  vast.ai listings, 6 h old, cheapest $0.90/h"), a median-price history chart per
  GPU, the per-provider quote table, and the live-price switch.
* Tests: 7 over parsing, GPU matching, percentiles, staleness, the three price
  sources and a provider outage; 3 over the endpoint, the history series and the
  D1 writes, against a stubbed database — including the case where migration
  `0003` has not been applied yet, which must degrade to reference rates rather
  than fail the Planner tab.

Open follow-ups it creates: price alerts on a dip, a spot break-even that knows
what a checkpoint costs, marketplace storage and egress in the budget (the
`storage_cost` is already captured with every quote), more marketplaces in the
feed, and pinning a saved plan to the observation it was priced from.
